...
| 🗞 뉴스 수집 | `ui/data_management_v2.py` → `render_collect` (카드뷰: 대분류 탭 키워드/포탈 + 출처칩 + 사진 카드 `_sc_*` + 기사 모달 `_news_modal_body`; `⚙ 수집 설정` 서브뷰 `_render_collect_settings`: 키워드·출처·이력. +`ui/data_management_render.py` 순수 빌더, `ui/data_health.py`) |
