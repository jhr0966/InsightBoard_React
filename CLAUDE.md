...
8. **작업 완료 보고 의무**: 모든 개발 지시가 끝나면 사용자에게 아래 3가지를 **반드시** 한 메시지로 정리해 안내한다 (생략 금지, 단순 정보 질문 응답은 제외).
   1. **무엇이 어떻게 개발됐는지** — 새/수정 파일, 추가/변경된 함수·필드, 핵심 동작 요약.
   2. **어떻게 조치됐는지** — 테스트 결과(N/N passed), 금지 패턴 검사, 커밋·푸시·PR 번호/링크/상태, CI 상태(queued/in_progress/success/failure), 충돌 발생 시 해소 방법.
   3. **다음 단계** — 권장 후속 작업 1~3건. 사용자가 바로 지시 가능한 형태.

   형식 예시:

   ```
   ✅ <PR 제목> — PR #N (Draft/Ready/Merged)
   변경: <파일·모듈·함수>
   조치: pytest N/N · CI <상태> · push <성공/실패>
   다음: <후속 작업 추천>
   ```
