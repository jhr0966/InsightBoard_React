...
def _isolated_data_dirs(monkeypatch, tmp_path):
    ...
    # store.sources 는 import 시 DATA_ROOT 를 from-import 함 (config.json 격리).
    try:
        import store.sources as sources_mod

        monkeypatch.setattr(sources_mod, "DATA_ROOT", root, raising=True)
    except ImportError:
        pass
