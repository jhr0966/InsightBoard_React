...
- 화면: 수집 관리 '빠른 수집'에 "비워 두고 수집하면 내 페르소나 관심 키워드로 모아요" 안내 추가.
- 검증: 신규 테스트 5건(`test_collect_keywords.py` — 명시 우선·페르소나 사용·상한·도메인 폴백·로드실패 폴백) 포함 pytest 607 passed · 웹 빌드 OK · 금지패턴 0.
...
- **`Collect.tsx` 관리 전용화**: 수집 실행·데이터 준비 상태·출처 헬스·수집 이력·기사 진단만 남김(운영자 화면).
- **홈 다이어트** (`Board.tsx`): 7섹션 대시보드 → **5요소 3분 화면**(인사말 → SOLA 한 줄 브리핑 → 개인화 다이제스트 → 자동화 제안 3장 → 내 키워드). KPI 4종·기회 매트릭스·트렌드 차트 2종·탑스토리 캐러셀 제거 — 매트릭스·트렌드는 분석실과 중복이었음.
- **분석실 탭 구조** (`Insights.tsx`): STEP1~3 세로 나열 → 탭(📈 트렌드→공정 / 🧭 기회 매트릭스 / 🔬 기술 히트맵) — 파고드는 층도 화면은 한 번에 하나.
- Topbar 전역검색 → `/feed` 로 이동(과거 /collect). SOLA 드로어 화면별 추천질문에 feed 추가·collect 는 관리 톤으로 조정.
- 검증: 프런트 전용(백엔드·OpenAPI 무변경) — pytest 580 passed · `npm run build`(tsc) OK · 금지패턴 0.
