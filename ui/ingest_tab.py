...
def render() -> None:
    page_header("뉴스 수집 + 본문 Enrich", "네이버 · 구글 · AI Times · 오토메이션월드")
    ...
    with btn_col2:
        if st.button("✨ 본문 Enrich (LLM 키워드/요약)", type="secondary"):
    ...
    st.markdown("---")
    df = load_all_today()
    st.caption(f"오늘 저장된 전체 기사: {len(df)}건  ·  enrich 완료: "
    ...
    st.markdown("**최근 10건 (enrich 결과 우선)**")
