...
>
> **메뉴 순서**(2026-06-15): 오늘의 보드 → 인사이트 분석 → 자동화 제안 → 뉴스 수집 → 작업 정의.
> (인사이트·산출물을 앞에, 데이터 운영(수집·정의)을 뒤로 — "소비 먼저, 관리 나중" 동선)

| 순 | 새 화면명 | 라우트 | 이전 | 결정 | 역할(한 문장) · 탭 구성 |
|---|---|---|---|---|---|
| 1 | **홈 / 오늘의 브리핑** | `/` | 오늘의 보드 | 유지·축소 | 페르소나 맞춤 "오늘 볼 것" 다이제스트. 심층 차트는 분석으로 이관 |
| 2 | **인사이트 분석** | `/insights` | 인사이트 분석 | 유지·강화 | 트렌드·매칭·자동화 기회 매트릭스 집중 |
| 3 | **자동화 제안** | `/proposals` | SOLA 작업실(개명) | 유지·**탭 추가** | 탭: `제안 생성`(채팅/작업대로 제안서 초안) · **`보관한 제안`**(채택·보관 제안서만) |
| 4 | **뉴스 수집** | `/collect` | 뉴스 수집 | 유지·**탭 추가** | 탭: `수집`(카드뷰) · **`보관한 뉴스`**(북마크 뉴스만) · `수집 설정`(키워드·출처·이력) |
| 5 | **작업 정의** | `/taskdefs` | 작업 정의 | 유지·폼 확정 | 엑셀 업로드 + JSON 폼 CRUD (→ 2단계) |
