...
|---|---|---|---|---|
| A | **백엔드 API 추출** | FastAPI로 `store/sola/roadmap/scraping` 래핑 + OpenAPI → React 타입드 클라이언트 | UI가 도메인 직호출 | `taskdefs`·`bookmarks` CRUD · `news`·`trends`·`proposals` · `assistant` SSE챗+context ✅ (OpenAPI 17경로) / collect 실행·scraping ⬜ |
