> 📜 **아카이브 (Streamlit 시절 불변식)** — 아래 I-1~I-23 은 은퇴한 Streamlit 런타임(st.rerun·session_state·fragment)에 대한 함정 모음이다. 현행 React/FastAPI 불변식은 `docs/INVARIANTS.md` 를 보라. HTTP/enrich/LLM/config 단일 진입점 등 일부는 여전히 유효(새 문서에 승계).
