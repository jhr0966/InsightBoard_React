...
## 4.5 P2 실행 방식 (결정 2026-06-16)
- **디자인 = 토큰 기반 클린 재작성**(Streamlit CSS 그대로 승계 X). 단 **UI/UX 완성도 최우선** — 현행 이상 수준으로.
- **화면 1개 = PR 1개**: 매 PR마다 기능 풀빌드 + 폴리시(타이포·간격·그림자·마이크로 인터랙션·로딩 스켈레톤·빈상태). 머지 시 그 화면은 "완성".
- 순서: **P1 마무리 → 보드 → 인사이트 → 수집(+설정) → 작업정의 → 보관함 → 페르소나/온보딩 → SOLA 작업실 → 어시스턴트 강화**.
