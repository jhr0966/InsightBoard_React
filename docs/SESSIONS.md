...
**조치**: web build OK, 프런트 전용. 사용자 지시("반응형 폴리시 진행").
...
**조치**: web build 통과(112모듈). 다음 P2: 페르소나 설정·온보딩.

---

## 2026-06-16 — P2 작업 정의 화면 풀빌드 (`feat-p2-taskdefs`)

**무엇을**: TaskDefs.tsx 재작성(KPI·업로드·검색·리스트·구조화 상세[섹션·공정연결·이력]·풀 편집폼 20필드·삭제). taskdefs.css, history 메서드.

**조치**: web build 통과. 다음 P2: 보관함(칸반).
...
**조치**: web build 통과(102모듈). 다음: P1 API 공백(persona·matches·board·trends weekly/emergence·sources·collect status/diagnose·summarize) → P2 화면 재현.
...
**조치**: 새 브랜치(main 기준), 문서 전용. 다음=Phase 0(디자인시스템·셸·공통컴포넌트·차트4종).
...
**어떻게**: ① 일별 14칸 자동 전환(누적 1~2주) + '신규 N건' 배지 + 선행 무데이터 버킷 트림(`_delta_info`) ② ×/추가 제거, [⚙ 키워드 설정] st.dialog(내 키워드·숨김 multiselect, 저장은 run 최상단 consume) ③ 프롬프트를 헤드라인+불릿 형식으로 재작성, `_sanitize_brief` 절단 방어, `.db-brief-greet-tx` 래퍼로 flex 텍스트 조각 깨짐 수정(원인: flex 직접 자식 텍스트 노드), 캐시 v2.

**검증**: pytest 961(신규 11) · Playwright — 모달 열기→추가→저장→칩 반영, 어노 '-14% 추세 관찰 중', 한눈요약 1줄+불릿.

**다음**: ① 매트릭스 빈 상태 개선(뉴스·작업정의 매칭 0건 안내 → 행동 유도) ② 브리핑 카드 주제 편중 완화 ③ 일별 모드 X축 라벨 밀도 자동 조정.

## 2026-06-11 — feat: 소스 표기 단일화 — 내부 ID('tech') 전면 제거 (`feat-source-labels`)

**무엇을**: 집계에 'tech' 노출(= AI Times·오토메이션월드 묶음 ID) 지적 → 소스가 보이는 모든 곳을 ① 키워드 뉴스 = 네이버 뉴스/구글 뉴스 ② 뉴스 포탈 = AI Times/오토메이션월드 체계로 통일.

**어떻게**: `ui/news_sources.py` 신설(source+press→라벨/분류/그라데이션, 저장 스키마 불변). run_daily 가 tech saved 에 `sites`(press 별 건수) 기록 → 집계 표가 사이트 행으로 분해. 출처 설정 기본 표시명 rename(네이버 기술/Google RSS→네이버 뉴스/구글 뉴스, disabled legacy 환산) + 키워드/포탈 그룹 캡션. 보드 탑스토리·브리핑 카드·LLM 프롬프트·인사이트 히트맵 상세·채팅 컨텍스트·수집잡/진행률/오류 전부 라벨 경유.

**검증**: pytest 950(신규 7: 라벨·그라데이션·sites 분해·legacy 환산) · Playwright — 수집/설정/보드 본문 'tech'·구라벨 0건.

**다음**: ① 관심 키워드 그래프 재설계(전부 +100% 문제) ② 보드 키워드 관리 UI 단순화 ③ 한눈요약 콘텐츠 재설계 — 사용자 지시 수신, 후속 브랜치.
## 2026-06-11 — fix: 온보딩 자동 포커스·Ctrl+Enter 실환경 레이스 (`fix-onboarding-focus-nav`)

**무엇을**: 사용자 보고 ① [시작하기] 후 이름 입력에 커서 미활성 ② Ctrl+Enter 로 [다음] 안 눌림 — 헤드리스 재현은 통과 → **실환경 타이밍 레이스**로 진단.

**어떻게** (`ui/components.py` `_FOCUS_NAV_JS`): ① 자동 포커스 폴링을 성공 즉시 중단하지 않고 5초간 재포커스 가드(추가 rerun 이 모달 DOM 교체 → 포커스 body 행, 마크업 동일이라 스크립트 재실행 없음). ② Ctrl+Enter 를 `e.target∈모달` 조건 → "모달 존재" 조건으로 전역화(포커스 body 여도 동작). ③ 고정 180ms 후 1회 클릭 → `clickWhenIdle`: 앱 idle(stStatusWidget 부재)+버튼 존재까지 대기 후 1회 클릭, in-flight 플래그로 연타 차단.

**검증**: pytest 943 · Playwright 8/8 (포커스 강탈 시뮬레이션 포함, `/tmp/onb_repro2.py`).

**다음**: ① 4단계 키워드 드롭다운 자동 닫기 ② 수집 제안 모달 키워드 칩 미리보기.
...
**어떻게**: `scraping/diagnose.py`(단계 구조화 + 200 위장 차단 감지) + ⚙ 수집 설정 '🔬 기사 URL 진단' 카드 + curl_cffi 미설치 경고 + enrich fetch 실패 로깅.

**검증**: pytest 928 passed · 금지패턴 0. (담당 에이전트 중단 → 오케스트레이터가 검증·문서·커밋 마무리)
