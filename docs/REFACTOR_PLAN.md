...
- 🟡 **silent except → 로깅 가드** (부분 완료) — ✅ `ui/_safe.guard(label)` 도구 + `data_management` 5곳 적용. **남음**: board_v2/insights_v2 ~40 site 롤아웃. [M]
...
### ⚪ 여력 시 (low)
✅ 매칭 `iterrows()`→`to_dict("records")` · ✅ `run_log._trim` 사이즈 게이트. **남음**: 의미매칭 엣지케이스 +2~3 · 라이브 수집 백그라운드+폴링.
