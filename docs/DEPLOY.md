...
## 3-1. 아침 자동수집 (`.github/workflows/daily-collect.yml`)

무료 플랜은 슬립·재배포 시 수집 데이터가 초기화된다("수시로 없어짐"). 이를 완화하려고
GitHub Actions 가 하루 여러 번(KST 07/13/18시) **배포된 백엔드의 `/api/collect` 를 HTTP 로
호출**해 백엔드가 자기 저장소에 뉴스를 채우게 한다(러너에서 수집하지 않는다 — 러너 디스크는
휘발이라 앱에 도달하지 못한다). 호출이 곧 웨이크업이라 사용 시간대 슬립도 준다.

설정:
- **repo variable `BACKEND_URL`** = 백엔드 URL (Settings → Secrets and variables → Actions →
  Variables). 미설정 시 워크플로 기본값을 쓴다.
- 수동 실행: Actions → daily-collect → Run workflow (keywords/max_results override 가능).
- 데이터 완전 영구화가 필요하면 §1의 Render 유료+디스크 또는 사내 서버/DB(Phase 2)로 전환.

> 구 `scrape-daily.yml`(러너에서 수집→repo commit)은 제거됐다. `data/news` 가 gitignore 라
> commit 이 no-op 이었고, 무엇보다 repo 커밋은 라이브 백엔드 저장소를 채우지 못한다.
