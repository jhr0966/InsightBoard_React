...
- **데드**: `app_shell.render_app_side` / `render_app_sola` 는 호출부 호환용 no-op (좌측은 네이티브 사이드바, 우측은 render_side 가 대체) — 호출부 제거는 후속 PR. `chat_panel.render`(구 bottom expander) 는 남아있으나 app.py 미사용. (`ui/layout.py`·`ui/task_tree.py`·`sola/{insight,chat_ctx}.py` 는 Phase 3 에서 삭제됨.)
